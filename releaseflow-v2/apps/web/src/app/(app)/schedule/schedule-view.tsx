...
/**
 * Operational schedule / calendar view (CE-007 / MUX-001 / MUX-002).
 *
 * Reused by:
 * - Calendar module (BUILD-220F) — Schedule + Releases tabs
 * - Releases → Schedule (BUILD-220B)
 * - Legacy /schedule (redirects to /calendar/schedule)
 *
 * Assignments remain the source of truth; this is a projection UI only.
 */
...
import { useCalendarEmbed } from '@/app/(app)/calendar/_components/calendar-embed-context';
...
/**
 * - full: Assignments | Releases domain switcher (Releases → Schedule workspace)
 * - assignments: operational schedule only (Calendar → Schedule)
 * - releases: release milestones only (Calendar → Releases)
 * - deadlines: agenda filtered to due today + overdue (Calendar → Deadlines)
 */
export type ScheduleSurface = 'full' | 'assignments' | 'releases' | 'deadlines';

export interface ScheduleViewProps {
  surface?: ScheduleSurface;
}

export default function ScheduleView({ surface = 'full' }: ScheduleViewProps) {
  const { user } = useAuth();
  const { activeOrgId } = useOrgStore();
  const { role } = useRoleStore();
  const embedded = useCalendarEmbed();
  ...
  const lockDomain = surface === 'assignments' || surface === 'releases' || surface === 'deadlines';
  const initialDomain: 'assignments' | 'releases' =
    surface === 'releases' ? 'releases' : 'assignments';

  // MUX-002.4 — domain tabs: Assignments | Releases (hidden when surface locks domain)
  const [domain, setDomain] = useState<'assignments' | 'releases'>(initialDomain);
  ...
  // Calendar → Deadlines: reuse existing assignment projections (no new domain)
  const deadlineItems = useMemo(() => {
    const overdue = applyScheduleFilters(items, { status: 'overdue', priority: 'all' });
    const dueToday = applyScheduleFilters(items, { status: 'due_today', priority: 'all' });
    const seen = new Set<string>();
    const merged: ScheduleAssignmentItem[] = [];
    for (const item of [...overdue, ...dueToday]) {
      if (seen.has(item.assignment.id)) continue;
      seen.add(item.assignment.id);
      merged.push(item);
    }
    return merged;
  }, [items]);
  const deadlineAgenda = useMemo(() => buildAgenda(deadlineItems), [deadlineItems]);

  const activeDomain = lockDomain ? initialDomain : domain;
  const showPageTitle = !embedded;
  const subtitle =
    surface === 'deadlines'
      ? 'Due today and overdue work from existing assignments.'
      : activeDomain === 'releases'
        ? 'Release milestones and dates across the label.'
        : 'What work needs to happen, when, and by whom.';

  return (
    <div
      className={`mx-auto w-full max-w-6xl px-4 sm:px-6 ${embedded ? 'pb-6' : 'py-6'} page-transition`}
      data-schedule-surface={surface}
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          ...
          <p className={`text-sm text-text-400 ${showPageTitle ? 'mt-0.5' : ''}`}>
            {subtitle}
          </p>
        </div>
        {surface !== 'deadlines' ? (
        ...
        <div className="rounded-2xl border border-surface-700/50 bg-surface-900/40 overflow-hidden">
          {loading ? (
            ...
            <p className="text-sm text-text-500 px-5 py-10 text-center">
              No due-today or overdue assignments right now.
            </p>
          ) : (
            <AgendaView sections={deadlineAgenda} />
          )}
        </div>
      ) : activeDomain === 'releases' ? (
        <div className="rounded-2xl border border-surface-700/50 bg-surface-900/40 overflow-hidden">
