...
function statusColor(status: string): string {
  switch (status) {
    case 'approved':
      return 'bg-success-500/15 text-success-500 dark:text-success-400';
    case 'in_review':
      return 'bg-info-500/15 text-info-500 dark:text-info-400';
    case 'changes_requested':
    case 'rejected':
      return 'bg-danger-500/15 text-danger-500 dark:text-danger-400';
    case 'draft':
      return 'bg-surface-200 dark:bg-surface-700 text-text-500 dark:text-text-400';
    case 'archived':
      return 'bg-surface-200 dark:bg-surface-700 text-text-400 dark:text-text-500';
    default:
      return 'bg-surface-200 dark:bg-surface-700 text-text-500 dark:text-text-400';
  }
}
  ...
  return (
    <div className={className}>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-surface-200 dark:border-surface-700 p-0.5 bg-layer-2 dark:bg-surface-900">
          <button
            onClick={() => setView('grid')}
            className={`p-1.5 rounded-md transition-colors ${view === 'grid' ? 'bg-surface-50 dark:bg-surface-800 shadow-sm' : 'text-text-400 hover:text-text-600'}`}
          ...
          <button
            ...
            className={`p-1.5 rounded-md transition-colors ${view === 'list' ? 'bg-surface-50 dark:bg-surface-800 shadow-sm' : 'text-text-400 hover:text-text-600'}`}
            ...
            return (
              <section key={group.key} aria-label={group.label}>
                ...
                {group.assets.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-3">
                    {group.assets.map((asset) => (
                      <button
                        ...
                        className="group relative flex flex-col rounded-xl border border-surface-200 dark:border-surface-700 bg-layer-2 dark:bg-surface-900 overflow-hidden hover:border-primary-500/50 hover:shadow-sm transition-all text-left"
                      >
                        ...
                        <div className="p-2.5 flex flex-col gap-1 min-w-0">
                          <p className="text-xs font-medium text-text-800 dark:text-text-200 truncate">{asset.title}</p>
                  ...
                  <div className="rounded-xl border border-surface-200 dark:border-surface-700 bg-layer-2 dark:bg-surface-900 shadow-card overflow-hidden">
                    {group.assets.map((asset, i) => (
                      <button
                        key={asset.id}
                        onClick={() => onSelect?.(asset)}
                        className={`w-full text-left flex items-center gap-3 px-4 py-3 hover:bg-surface-50 dark:hover:bg-surface-800/40 transition-colors duration-100 ${
                          i > 0 ? 'border-t border-surface-100 dark:border-surface-800' : ''
                        }`}
                      >
                        <div className="h-10 w-10 rounded-lg bg-surface-100 dark:bg-surface-800 flex items-center justify-center shrink-0 overflow-hidden">
                          {asset.thumbnailUrl ? (
                        ...
                        <div className="flex-1 min-w-0 grid grid-cols-2 sm:grid-cols-4 gap-2 items-center">
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-text-800 dark:text-text-200 truncate">{asset.title}</p>
                        ...
                        <svg className="h-4 w-4 text-text-300 dark:text-text-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
