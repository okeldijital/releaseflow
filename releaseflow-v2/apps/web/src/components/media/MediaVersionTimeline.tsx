...
export function MediaVersionTimeline({
  ...
  if (versions.length === 0) {
    return (
      <div className={`flex flex-col items-center justify-center py-16 text-center ${className}`}>
        ...
        <p className="text-sm font-medium text-text-700 dark:text-text-300">No versions yet</p>
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="absolute left-[17px] top-3 bottom-3 w-0.5 bg-surface-200 dark:bg-surface-700" aria-hidden="true" />
              ...
              <div className="relative flex flex-col items-center">
                <div
                  className={[
                    ...
                    isCurrent
                      ? 'bg-primary-500 text-surface-50 ring-surface-0 dark:ring-surface-950'
                      : 'bg-surface-200 dark:bg-surface-700 text-text-500 dark:text-text-400 ring-surface-0 dark:ring-surface-950',
                  ].join(' ')}
                >
              ...
              <div
                className={[
                  ...
                  isCurrent
                    ...
                    : 'border-surface-200 dark:border-surface-700 bg-layer-2 dark:bg-surface-900',
                ].join(' ')}
              >
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="text-sm font-semibold text-text-900 dark:text-text-100">
                        Version {version.versionNumber}
                      </h3>
                      {isCurrent && (
                        <span className="inline-flex items-center rounded-full bg-primary-500/15 text-primary-500 dark:text-primary-400 px-2 py-0.5 text-caption font-medium">
                          Current
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-500">
                  <span><span className="text-text-400 dark:text-text-500 font-medium">Size:</span> {formatSize(version.fileSize)}</span>
                  {version.dimensions && (
                    <span><span className="text-text-400 dark:text-text-500 font-medium">Dimensions:</span> {version.dimensions.width} × {version.dimensions.height}</span>
                  )}
                  <span><span className="text-text-400 dark:text-text-500 font-medium">Type:</span> {version.mimeType}</span>
                </div>

                {version.notes && (
                  <p className="mt-2 text-sm text-text-600 dark:text-text-400 bg-surface-50 dark:bg-surface-800 rounded-lg px-3 py-2">
                    {version.notes}
                  </p>
                )}
                ...
                {canCompare && idx < sorted.length - 1 && (
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
                  >
